...
(defun ext-id-of-node (node)
  ;; the index of internal line can be changed, the index of external
  ;; line should be fixed.  An index is arbitary index When it equals to 0.
  (let ((lo (out-line-of node))
        (li (in-line-of node)))
    (+ (* 100 (+ (* 4 (symb-id-of-line lo)) (symb-id-of-line li)))
       (+ (* 4 (if (internal-line? lo)
                   (index-id-of-line lo)
                   0))
          (if (internal-line? li)
              (index-id-of-line li)
...
(defun amp-id-fn (node-fn)
  (lambda (amp)
    (sort (mapcar node-fn (content-of amp))
          #'<)))
(defun ampprod-id-fn (node-fn)
  (lambda (ampprod)
    (sort (mapcar (id-fn-of-amp node-fn) ampprod)
          (lambda (id1 id2)
            (let ((len1 (length id1))
                  (len2 (length id2)))
              (if (eql len1 len2)
...
;        #'<))
;(defun ext-id-of-amp (amp)
;  (sort (mapcar #'ext-id-of-node (content-of amp))
;        #'<))
...
(defun replace-line-for-amp (fn amp)
  (print amp)
  (cons (tag-of amp)
        (mapcaar (lambda (line) (funcall fn line))
                 (content-of amp))))
(defun swap-line-in-ampprod (line1 line2 ampprod)
  (flet ((replace-fn (line)
           (cond ((equal line line1) line2)
                 ((equal line line2) line1)
                 (t line))))
    (mapcar (lambda (amp) (replace-line-for-amp #'replace-fn amp))
            ampprod)))
...
(defun remove-symm-eq-ampprod (ampprod-lst)
  (remove-duplicates ampprod-lst
                     :test (lambda (a1 a2)
                             (equal (funcall (ampprod-id-fn #'id-of-node) a1)
                                    (funcall (ampprod-id-fn #'id-of-node)
                                             (swap-ampprod-e1<->e2 a2))))
                     :from-end t))
  ...
  (let ((op (first ampprod))
    ...
    (when (symmetric-op? op)
      ...
      (let ((swapped (swap-ampprod-e1<->e2 rest-amps)))
        (when (equal (funcall (ampprod-id-fn #'id-of-node) swapped)
                     (funcall (ampprod-id-fn #'id-of-node) rest-amps))
          (not
           ...
           (and (equal (funcall (amp-id-fn #'symb-id-of-node) op)
...
(defun count-inner-loops (ampprod)
  (let ((op (first ampprod))
    ...
    (flet ((find-node-fn (node-patten)
      ...
      (cond ((member (funcall (amp-id-fn #'symb-id-of-node) op)
                     (mapcar (amp-id-fn #'symb-id-of-node)
                             '((f ((hi- 1) (pi- 1)))
